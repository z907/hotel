...
public class CheckService:BaseService
{
    ...
    public bool CheckForFreeRooms(DisplayReservation toCheck)
    {
        ...
        List<Reservation> resList = db.Reservations.Where(r => r.RoomAttributesId == FoundAttr.Id).Where(r=>r.Status.Status=="Активно" || r.Status.Status=="Оплачено" || r.Status.Status=="Не оплачено" ).ToList();
        bool flag = false;
        int counter = 0;
    ...
    public bool CheckForFreeRoomsUpdate(DisplayReservation toCheck)
    {
        RoomAttribute attr = new RoomAttribute();
        attr.CapacityId = db.RoomCapacities.Where(c=>c.Value==toCheck.RoomCapacity).Select(c=>c.Id).FirstOrDefault();
        attr.QualityId = db.RoomQualities.Where(q=>q.Name==toCheck.RoomQuality).Select(q=>q.Id).FirstOrDefault();
        ...
        int roomcount = db.Rooms.Where(r=>r.RoomAttributesId==FoundAttr.Id).Count();
        db.ReservationStatuses.Load();
        List<Reservation> resList = db.Reservations.Where(r => r.RoomAttributesId == FoundAttr.Id && r.Id!=toCheck.Id).Where(r=>r.Status.Status=="Активно" || r.Status.Status=="Оплачено" || r.Status.Status=="Не оплачено" ).ToList();
        bool flag = false;
        int counter = 0;
